...
2. **Важно: выберите Node.js.** В настройках бота на BotHost укажите:
   - **Язык/рантайм: Node.js** (не Python). Иначе хостинг соберёт проект как Python-бота (pip, pyTelegramBotAPI) и бот не запустится.
   - **Главный файл запуска:** `bot.js` или команда: `node bot.js` / `npm start`.
   - В репозитории есть **Dockerfile** — если хостинг поддерживает сборку по Dockerfile, он соберёт образ на Node.js и запустит `node bot.js`.

3. **Переменные окружения.** В панели хостинга задайте:
   - `BOT_TOKEN` — токен бота из Discord Developer Portal (или `DISCORD_TOKEN`, бот поддерживает оба имени).

4. **Конфигурация.** На хостинге должен быть файл `config.json` с вашими ID сервера, каналов и ролей. Варианты:
...
6. **Node.js.** Убедитесь, что на хостинге выбран Node.js 18 или выше.
